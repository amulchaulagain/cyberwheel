"""Benchmark: full RL env steps/second with green noise on the 15-host network.

Standalone script (see bench_network_build.py for the contract). Complements
bench_rl_step (green off, perfect detector) by exercising the flagship
green-noise path via green_noise_vs_rl_blue.yaml: scripted green sessions,
a multi-alert detector stream through the noisy NIDS stack (the
DetectorHandler dedup this benchmark exists to watch), the active-defense
blue action set, and the availability-aware reward. On revisions that predate
the flagship config the perf gate's ``--compare-rev`` skips this benchmark
there and reports its metric as NEW.
...
def build_env(seed: int):
    ...
    args = YAMLConfig("green_noise_vs_rl_blue.yaml")
    args.parse_config()
    args.seed = seed
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "metric": "rl_step_green_sps_15host",
